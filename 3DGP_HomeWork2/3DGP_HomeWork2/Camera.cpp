...
void CCamera::SetViewport( int xTopLeft, int yTopLeft, int nWidth, int nHeight, float fMinZ, float fMaxZ )
{
	...
	m_d3dViewport.MinDepth = float( fMinZ );
	m_d3dViewport.MaxDepth = float( fMaxZ );
}
...
void CCamera::RegeneratreViewMatrix()
{
...
CFirstPersonCamera::CFirstPersonCamera( CCamera* pCamera ) : CCamera(pCamera)
{
	...
	if ( pCamera )
	{
		/*1인칭 카메라로 변경하기 이전의 카메라가 스페이스-쉽 카메라이면 카메라의 Up벡터를 월드좌표의 y-축이 되도록 한다.
		...
		y-좌표가 0.0f가 되도록 한다. 이것은 다음 그림과 같이 로컬 x-축 벡터와 로컬 z-축 벡터를 xz-평면(지면)으로 투영하는 것을 의미한다.
...
void CThirdPersonCamera::Update( XMFLOAT3& xmf3LookAt, float fTimeElapsed )
{
	...
	if ( m_pPlayer )
	{
		...
		m_fTimeLag가 1보다 크면 fTimeLagScale이 작아지고 실제 회전(이동)이 적게 일어날 것이다. m_fTimeLag가 0이 아닌 경우 fTimeElapsed를 곱하고 있으므로
